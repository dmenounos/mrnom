...
Texture* ResourceFactory::createTexture(const char* path) {
	...
	ResourceReader resourceReader(assetManager, path);
	...
	resourceReader.read(header, sizeof(header));
	...
	png_set_read_fn(pngPtr, &resourceReader, readCallback);
...
void readCallback(png_struct* pngPtr, png_byte* pngData, png_size_t pngSize) {
	ResourceReader* reader = (ResourceReader*) png_get_io_ptr(pngPtr);
